...
import lobby.handlers.GameCompletedHandler;
...
import lobby.handlers.LobbyRoomsChangedHandler;
...
import lobby.handlers.NewKingHandler;
import lobby.handlers.NewMasterHandler;
...
import lobby.handlers.SetSlotOrderHandler;
...
//			break;
	...
	public void onLeaveRoom(UserSession userSession, boolean isDisconnected) throws IOException, SQLException {
		Room room = getRoom(userSession.getUser().getRoomIndex());
		
		userSession.sendMessage(new LobbyRoomsChangedHandler(this, userSession).getResponse(room));
		sendBroadcastMessage(userSession, new LobbyRoomsChangedHandler(this, userSession).getResponse(room));
	
		if (room.getNumberOfPlayers() == 0) {
			setRoom(room.getRoomID(), null);
		}
		else {
			if (room.getGameMode() == GameMode.FIGHT_CLUB) {
				if (userSession.getUser().isInGame()) {
					for (int i = room.getOrderBySlot(userSession.getUser().getRoomSlot()); i < room.fightClubOrder.length - 1; i++) {
						room.fightClubOrder[i] = room.fightClubOrder[i + 1];
					}
					room.inGamePlayers--;
					sendRoomMessage(userSession, new SetSlotOrderHandler(this, userSession).getResponse(room.fightClubOrder), false);
				}
			}
			else if (room.getGameMode() == GameMode.TOURNAMENT) {
				// this should only happen when the game has started
				if (room.isStarted) {
					int[] order = userSession.getUser().getRoomTeam() == 10 ? room.blueTeamOrder : room.redTeamOrder;
					int winningTeam = userSession.getUser().getRoomTeam() == 10 ? 20 : 10;
					
					for (int i = room.getOrderBySlot(userSession.getUser().getRoomSlot(), userSession.getUser().getRoomTeam()); i < 3; i++) {
						order[i] = order[i + 1];
					}
					
					sendRoomMessage(userSession, new SetSlotOrderHandler(this, userSession).getResponse(room.blueTeamOrder, room.redTeamOrder), false);
					...
					if (order[0] == -1) {
						...
						sendRoomMessage(userSession, new GameCompletedHandler(this, userSession).getResponse(results, winningTeam), false);
					}
				}
			}
			...
			if (room.masterIndex == userSession.getUser().getRoomSlot()) {
				...
				sendRoomMessage(userSession, new NewMasterHandler(this, userSession).getResponse(room.masterIndex), true);
			}
			if (room.blueKingIndex == userSession.getUser().getRoomSlot()) {
				room.blueKingIndex = room.getAnyPlayerSlot();
				sendRoomMessage(userSession, new NewKingHandler(this, userSession).getResponse(), true);
			}
			...
			if (room.getGameMode() == GameMode.HERO && userSession.getUser().getRoomSlot() == room.heroSlot) {
				...
				sendRoomMessage(userSession, new GameCompletedHandler(this, userSession).getResponse(results, 0), false);
			}
		}
