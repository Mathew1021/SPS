...
public class GoldForceChargeHandler extends LobbyHandler {
	...
	public void processMessage() throws SQLException {
		...
		if (card.getCardPremiumExpirationDate() == null) {
			Date date = new Date();
			date.setTime(System.currentTimeMillis() + (long) premiumDays * 24 * 60 * 60 * 1000);
			card.setCardPremiumExpirationDate(date);
		}
		else {
			card.getCardPremiumExpirationDate().setTime(card.getCardPremiumExpirationDate().getTime() + (long) premiumDays * 24 * 60 * 60 * 1000);
		}
