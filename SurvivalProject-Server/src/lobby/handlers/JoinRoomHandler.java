...
public class JoinRoomHandler extends LobbyHandler {
	...
	public byte[] getResponse() {
		...
		output.putInt(0x5C, 2); // character? nah
		output.putByte(0x60, (byte) 0); // like field f4. 0 - don't start. anything else - start
		output.putByte(0x61, (byte) 0); // master card
		...
		output.putShort(0x74, (short) 10); // same as 0x10C in RoomPlayersUpdate. it's aura.
		output.putBoolean(0x78, lobbyServer.getRoom(roomID).isLimitAnger());
		output.putByte(0x79, (byte) 0); // same as field 6b in create room
