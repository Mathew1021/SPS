...
public class JoinLobbyHandler extends LobbyHandler {
	...
	// When you log in you get a visit bonus. This is set to 0 because we'll be sending the visit bonus on a different message.
    // It gives us better control on the timing.
	int visitBonusMoney = 0;
	int visitBonusElementsType = 0;
	int visitBonusElements = 0;
	int visitBonusElementsMultiplier = 0;
	int visitBonusAvatarMoney = 0;
	...
	public JoinLobbyHandler(LobbyServer lobbyServer, UserSession userSession, byte[] messageBytes) {
	...
	public void processMessage() throws SQLException, IOException {
		...
		lobbyServer.moveToCorrectPlace(userSession);
		...
		// Load player rank
		playerRank = userSession.getUser().getRank();
		
		if (userSession.getUser().getLastVisitBonus() == 0 || System.currentTimeMillis() - userSession.getUser().getLastVisitBonus() > 4 * 60 * 60 * 1000) {
			visitBonusElementsType = (int) (Math.random() * 4) + 1;
			visitBonusElements = (int) (Math.random() * 5) + 1;
			visitBonusElementsMultiplier = (int) (Math.random() * 4) + 1;
			visitBonusMoney = 1000;
			visitBonusAvatarMoney = (int) (Math.random() * 20) + 1;
			
			if (userSession.getUser().hasCard(Card.VISIT_BONUS)) {
				visitBonusElementsMultiplier *= 2;
				visitBonusMoney *= 2;
				visitBonusAvatarMoney *= 2;
				
				int i;
				for (i = 0; i < 96; i++) {
					if (userSession.getUser().getCard(i) != null && userSession.getUser().getCard(i).getCardID() == Card.VISIT_BONUS) {
			...
			userSession.getUser().setWhiteCard(visitBonusElementsType - 1, userSession.getUser().getWhiteCard(visitBonusElementsType - 1) + visitBonusElements * visitBonusElementsMultiplier);
			userSession.getUser().setPlayerCode(userSession.getUser().getPlayerCode() + visitBonusMoney);
			userSession.getUser().setAvatarMoney(userSession.getUser().getAvatarMoney() + visitBonusAvatarMoney);
			userSession.getUser().setLastVisitBonus(System.currentTimeMillis());
