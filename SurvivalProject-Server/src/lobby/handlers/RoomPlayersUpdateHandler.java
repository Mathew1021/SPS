...
//			}
			...
			if (room.getGameMode() == GameMode.FIGHT_CLUB) {
				lobbyServer.sendRoomMessage(userSession, new SetSlotOrderHandler(lobbyServer, userSession).getResponse(room.fightClubOrder), true);
			}
			if (room.getGameMode() == GameMode.TOURNAMENT) {
				int blueCounter = 0, redCounter = 0;
				for (int i = 0; i < 8; i++) {
					room.blueTeamOrder[i] = -1;
					room.redTeamOrder[i] = -1;
					
					if (room.getUserSession(i) != null) {
						if (room.getUserSession(i).getUser().getRoomTeam() == 10) {
							room.blueTeamOrder[blueCounter++] = i;
						}
						else {
							room.redTeamOrder[redCounter++] = i;
						}
					}
				}
				
				lobbyServer.sendRoomMessage(userSession, new SetSlotOrderHandler(lobbyServer, userSession).getResponse(room.blueTeamOrder, room.redTeamOrder), true);
			}
			
			if (room.getGameMode() == GameMode.MISSION || room.getGameMode() == GameMode.BIG_MATCH_DEATH_MATCH || room.getGameMode() == GameMode.CRYSTAL) {
				lobbyServer.sendRoomMessage(userSession, new SetSlotOrderHandler(lobbyServer, userSession).getResponse(), true);
			}
			
			if (room.getGameMode() == GameMode.HOKEY) {
				lobbyServer.sendRoomMessage(userSession, new SetSlotOrderHandler(lobbyServer, null).getResponse(), true);
//				lobbyServer.sendRoomMessage(userSession, new SoccerGoalHandler(lobbyServer, null).getResponse(room, 3), true);
			}
...
//				lobbyServer.sendRoomMessage(userSession, new SetSlotOrderHandler(lobbyServer, userSession).getResponse2(), true);
				room.blueKingIndex = 0;
...
//		output.putString(0x18, "10.0.0.50"); // IP!!!
		...
		output.putInt(0x78, 1); // same as unknown04 in the request. if this is -1, there are no characters?
		...
		output.putInt(0xD0, 30); // between 1 and 16. There are 12 characters + 4 avatar types. Maybe it's related
		output.putInt(0xD4, 30); // same ^
...
//		int slot = userSession.getUser().roomSlot;
		...
		if (room.getGameMode() == GameMode.FIGHT_CLUB) {
			if (userSession.getUser().getRoomReady() == 1) {
				room.fightClubOrder[room.inGamePlayers++] = userSession.getUser().getRoomSlot();
			}
		}
	}
}
