...
//		System.out.println(Cryptography.getDigest(bytes2));
	...
	public static void add(LobbyServer lobby, String name) throws IOException {
		UserSession userSession = new UserSession(lobby, null);
		userSession.getUser().setUsername(name);
		lobby.getUserSessions().add(userSession);
		lobby.moveToCorrectPlace(userSession);
	}
